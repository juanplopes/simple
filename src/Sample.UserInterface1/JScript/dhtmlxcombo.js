...
function dhtmlXComboFromSelect(parent, size) {
    ...
    z.width = size;
    $(z).css('float', 'left').css('margin-right', '-3px').css('position', 'relative');
    z.className = "dropdown";
    z.id = parent.name + "_span";
...
dhtmlXCombo.prototype.getOptionByLabel = function(value) {
    ...
    for (var i = 0; i < this.optionsArr.length; i++) if (this.optionsArr[i].text == value) return this.optionsArr[i];
    return null
};
...
dhtmlXCombo.prototype._createSelf = function(selParent, name, width, tab) {
    ...
    this.DOMlist.onkeydown = function(e) {
        this.combo.DOMelem_input.focus(); (e || event).cancelBubble = true;
...
dhtmlXCombo.prototype._onKey = function(e) {
    var that = this.parentNode.combo; (e || event).cancelBubble = true;
    ...
    if ((ev != "13") && (ev != "9")) {
        ...
        that.closeAll(); (e || event).cancelBubble = false
    }
};
