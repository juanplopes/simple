...
namespace Simple.Metadata
{
    public class VistaDBSchemaProvider : DbSchemaProvider
    {
        ...
        public override string GetDatabaseName()
        {
            ...
            var dataSource = GetConnection().DataSource;

            Regex RegExp = new Regex(@"\\(?<db>[^\\]*)?\.vdb3$", RegexOptions.IgnoreCase);
            if (RegExp.IsMatch(dataSource))
            {
                Match found = RegExp.Matches(dataSource)[0];
        ...
        public override System.Data.DataTable GetConstraints()
        {
            ...
            DataTable Constraints = GetConnection().GetSchema("ForeignKeyColumns");
